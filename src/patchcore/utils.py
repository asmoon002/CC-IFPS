...
def compute_and_store_final_results(
    ...
    for i, result_key in enumerate(column_names):
        ...
        LOGGER.info("{0}: {1:.4f}".format(result_key, mean_metrics[result_key]))
    ...
    with open(savename, "w") as csv_file:
        ...
        for i, result_list in enumerate(results):
            ...
            formatted_row = [float(f"{val:.4f}") if isinstance(val, (int, float)) else val for val in result_list]
            csv_row = formatted_row
            if row_names is not None:
                csv_row = [row_names[i]] + formatted_row
            csv_writer.writerow(csv_row)
        mean_scores = [float(f"{val:.4f}") if isinstance(val, (int, float)) else val for val in list(mean_metrics.values())]
        if row_names is not None:
            mean_scores = ["Mean"] + mean_scores
        csv_writer.writerow(mean_scores)

    LOGGER.info("Results saved to CSV: {}".format(savename))
